...
`replaceCallbackQueryMessage: true` shows the ephemeral in place of the message the button belonged to, for that one user. group-only in practice — the same call in a private chat comes back `Bad Request: MESSAGE_ID_INVALID`, which fits the feature: a 1:1 chat has nobody to hide the original from. also measured: `editEphemeralMessageMedia` is rejected server-side with `MESSAGE_EMPTY` for every media form (file_id, url, fresh upload), while `editEphemeralMessageText` / `editEphemeralMessageCaption` work on the same target.
